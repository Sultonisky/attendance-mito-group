...
/**
 * Admin manual update of an outsource daily attendance record (no GPS/face).
 * Outsource + attendance_date stay locked; rebuilds the primary session/events.
 */
class UpdateOutsourceAttendance implements Action
{
    ...
    public function execute(AttendanceRecord $record, array $input, ?User $actor, ?Request $request = null): AttendanceRecord
    {
        if ($record->outsource_id === null || $record->attendable_type !== 'outsource') {
            throw new InvalidArgumentException('Not an outsource attendance record.');
        }

        return DB::transaction(function () use ($record, $input, $actor, $request): AttendanceRecord {
            /** @var Outsource $outsource */
            $outsource = Outsource::query()->findOrFail((int) $record->outsource_id);
            ...
            $before = [
                ...
                'pin_id' => $record->events()->where('event_type', AttendanceEventType::CheckIn->value)->value('work_location_pin_id'),
            ];

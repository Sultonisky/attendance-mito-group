...
async function submit(): Promise<void> {
  ...
  try {
    ...
    if (isAdmin) {
      await auth.logout()
      errorMessage.value = 'This account belongs to the admin console. Use the admin sign-in.'
      return
    }

    // Anchor this tab as the employee portal before navigating.
    setPortal('employee')
    await router.push({ name: 'employee-app' })
  } catch (error) {
...
<template>
  <main class="login">
    <section class="login-visual" aria-label="MITO Employee App">
      <div class="visual-content">
        <div class="logo-frame">
          <img class="brand-logo" src="/images/mito.png" alt="MITO electronic" />
        </div>
        <p class="visual-kicker">EMPLOYEE APP</p>
        <h1>Your day,<br /><strong>on record.</strong></h1>
        <p class="visual-copy">A clear, reliable way to record your attendance wherever you work.</p>
      </div>
    </section>
...
<style scoped>
/* ── Accent tokens — Employee: MITO Red ──────────────────────────── */
.login {
  --accent:        #eb1c24;
  --accent-dark:   #c9151c;
  --accent-bg:     #fff1f2;

...
async function load(): Promise<void> {
  ...
  try {
    const response = await fetchPenaltyReport({
      from: filters.from,
      to: filters.to,
    ...
    if (err instanceof ApiError) {
      if (err.status === 401) {
        await router.push({ name: 'login', query: { redirect: '/reports/penalties' } })
        return
      }
    ...
    error.value = 'Unable to load penalty report. Please try again.'
  } finally {
    loading.value = false
  }
}
  ...
  <main class="report-page">
    <header class="report-header">
      <div class="header-left">
        ...
        <span class="header-active">Penalties</span>
      </div>
    ...
    <section class="report-body">
        ...
        <table class="report-table">
          ...
          <tbody>
            <tr v-for="row in data" :key="row.id">
              ...
              <td>{{ row.penalty_rule_name }}</td>
              <td>{{ row.source }}</td>
              <td>{{ row.violation_type }}</td>
              <td>{{ row.original_points }}</td>
              <td>{{ row.adjusted_points }}</td>
              <td>{{ row.final_points }}</td>
              <td>{{ row.status }}</td>
              <td>{{ new Date(row.occurred_at + 'Z').toLocaleString() }}</td>
            </tr>
          </tbody>
        </table>
      ...
      <section v-else-if="!loading" class="report-empty">
        <p>No penalty records found for the selected filters.</p>
      </section>

      <section v-else class="report-loading" aria-label="Loading penalty report">
        <p>Loading penalty report...</p>
      </section>
    </section>
  </main>
</template>
